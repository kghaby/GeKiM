...
def chi_squared(observed_data, fitted_data, fitted_params, variances=None, reduced=False):
    ...
    Args:
        ...
        fitted_data (np.ndarray): The fitted data points obtained from curve 
        fitted_params (list or np.ndarray): The optimal parameters obtained from curve 
...
class _FitOutput:
    ...
    Attributes:
        ...
        fitted_data (ndarray): Data generated by the fitted params.
        reduced_chi_sq (float): Reduced chi-squared value indicating goodness of fit.
    """
    def __init__(self, fitted_params, pcov, fitted_data, reduced_chi_sq):
        self.fitted_params = fitted_params
        self.pcov = pcov
        self.fitted_data = fitted_data
        self.reduced_chi_sq = reduced_chi_sq

def _prepare_output(popt, pcov, param_order, fitted_data, observed_data):
    """
    Prepare the fitting output as an instance of the FitOutput class.
    """
    fitted_params = dict(zip(param_order, popt))
    reduced_chi_sq = chi_squared(observed_data, fitted_data, popt, np.var(observed_data), reduced=True)
    return _FitOutput(fitted_params, pcov, fitted_data, reduced_chi_sq)
